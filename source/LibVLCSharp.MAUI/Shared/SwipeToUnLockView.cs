...
namespace LibVLCSharp.MAUI.Shared
{
    ...
    public class SwipeToUnLockView : AbsoluteLayout
    {
        ...
        internal async void OnPanGestureUpdated(object sender, PanUpdatedEventArgs e)
        {
            ...
            switch (e.StatusType)
            {
                ...
                case GestureStatus.Running:
                    ...
                    SetLayoutBounds(view: FillBar, new Rect(0, 0, x + Thumb.Width / 2, Height));
                    break;

                case GestureStatus.Completed:
                    var posX = Thumb.TranslationX;
                    SetLayoutBounds(view: FillBar, new Rect(0, 0, 0, Height));
        ...
        internal void OnSizeChanged(object sender, EventArgs e)
        {
            ...
            SetLayoutFlags(view: TrackBar, AbsoluteLayoutFlags.SizeProportional);
            SetLayoutBounds(view: TrackBar, new Rect(0, 0, 1, 1));
            Children.Add(TrackBar);

            SetLayoutFlags(view: FillBar, AbsoluteLayoutFlags.None);
            SetLayoutBounds(view: FillBar, new Rect(0, 0, 0, Height));
            Children.Add(FillBar);

            SetLayoutFlags(view: Thumb, AbsoluteLayoutFlags.None);
            SetLayoutBounds(view: Thumb, new Rect(0, 0, Width / 5, Height));
            Children.Add(Thumb);

            SetLayoutFlags(view: _gestureListener, AbsoluteLayoutFlags.SizeProportional);
            SetLayoutBounds(view: _gestureListener, new Rect(0, 0, 1, 1));
