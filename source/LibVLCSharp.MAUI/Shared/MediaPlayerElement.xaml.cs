...
public partial class MediaPlayerElement : ContentView
{
    ...
    private void PageAppearing(object? sender, Page e)
    {
        if (e == this.FindAncestor<Page?>())
        {
            ...
            {
                //IDictionary<string, object> applicationProperties = Application.Current.Properties;
            ...
            MessagingCenter.Subscribe<LifecycleMessage>(this, "OnResume", m =>
            {
                ...
                if (mediaPlayer != null)
                {
                    ...
                    var play = Preferences.Get($"VLC_{mediaPlayer.NativeReference}_MediaPlayerElement_IsPlaying", false);
                    if (play is true)
                    {
                        mediaPlayer.Play();
                        var position = Preferences.Get($"VLC_{mediaPlayer.NativeReference}_MediaPlayerElement_Position", 0f);
